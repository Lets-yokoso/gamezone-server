...
app.post('/api/groups/:groupId/pcs', authMiddleware, async (req, res) => {
  try {
    ...
    const id = uuidv4();
    // FIX: Include payment_status field
    await db.insert('pcs', { 
      id, 
      group_id: groupId, 
      name, 
      ...
      session_end: 0, 
      stopwatch_start: 0, 
      payment_status: null,  // ← Added
      price_per_hour: price_per_hour || 0,
      order: Date.now()  // ← Added for reordering
    });
    res.json({ 
      id, 
      name, 
...
app.post('/api/pcs/:pcId/session/start', authMiddleware, async (req, res) => {
  try {
    ...
    io.to(`group:${group_id}`).emit('group:'+group_id+':pc-session', { 
      ...
      session_end, 
      stopwatch_start: 0,
      payment_status: pc.payment_status  // ← Added
    });
...
app.post('/api/pcs/:pcId/session/end', authMiddleware, async (req, res) => {
  try {
    ...
    io.to(`group:${group_id}`).emit('group:'+group_id+':pc-session', { 
      ...
      session_end: 0, 
      stopwatch_start: 0,
      payment_status: undefined  // ← Added
    });
